...
if(isset($_POST['submit'])){
    ...
    for($i=0;$i<count($_POST['id']);$i++){
        ...
        $proid=$_POST['pid'][$i];
        $fname=$_POST['fname'][$i];
        $price=$_POST['pricen'][$i];
        $name = $_POST['name'][$i];
        $qun=$_POST['quantity'] [$i];
        $d=date('d-m-Y');
        $order="INSERT INTO `order_tbl`(`login_id`, `cartid`, `price`,`status`,`date`) VALUES ('$uid', '$bid', '$price','unpaid','$d')";
         $stmt=$con->prepare($order);
         $stmt->execute();
         $query="SELECT quantity FROM farmer_product WHERE pro_id='$proid' and fname='$fname' ";
         $result=mysqli_query($con,$query);
         $data=mysqli_fetch_array($result);
            $dat=$data['quantity'];
            $total=0;
            $total=$dat-$qun;
            $n=mysqli_query($con,"UPDATE farmer_product set quantity='$total' where pro_id='$proid' and fname='$fname'");
            $sqlls = "UPDATE tbl_cart SET status='2' WHERE id='$bid'";
            mysqli_query($con,$sqlls);
...
<body>
    ...
    <div class="cart-box-main">
                                ...
                                <td class="name-pr">
                                   ...
                                   <?php echo $f; ?>
                                    ...
                                    <td class="name-pr">
                                        ...
                                        <input type="hidden" name="name[]" value="<?php echo $raw['product_name']; ?>" >
                                        <input type="hidden" name="pid[]" value="<?php echo $raw['pro_id']; ?>" >
                                        <input type="hidden" name="fname[]" value="<?php echo $raw['fname']; ?>" >
                                    ...
                                    <input type="hidden" name="pricen[]" value="<?php echo $raw['price']; ?>" > 
        ...
        <div class="container">
            ...
            <input type="submit" name="submit" value="order now" class="btn btn-outline-success btn-block " style="    width: 190px; margin-left: 10px;" >
            </form>
